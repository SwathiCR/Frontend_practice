...
function clearAll() {
  ...
  sections.forEach((section) => {
    // Remove all child elements except the heading (h3)
...
function showMeme() {
  ...
  let memeImgElem = createChildElements("memes", "memeImgId", "img");
  memeImgId.setAttribute("src", randomMemeUrl);
}
...
function showJoke() {
  ...
  let jokesParaElem = createChildElements("jokes", "jokesParaId", "p");
  jokesParaElem.textContent = randomJokeText;
}
...
function showQuote() {
  ...
  let quotesParaElemQuotes = createChildElements(
    "quotes",
    "quotesParaQuoteId",
    "p"
  );
  let quotesParaElemAuthor = createChildElements(
    "quotes",
    "quotesParaAuthorId",
    "p"
  );

  quotesParaElemQuotes.textContent = randomQuote.quote;
  quotesParaElemAuthor.textContent = "- " + randomQuote.author;
}
...
function showRiddle() {
  ...
  let riddleParaElemQuestion = createChildElements(
    "riddles",
    "riddlesParaQuestionId",
    "p"
  );
  let riddleParaElemAnswer = createChildElements(
    "riddles",
    "riddlesParaAnswerId",
    "p"
  );
  riddleParaElemQuestion.textContent = randomRiddle.question;
  riddleParaElemAnswer.textContent = "- " + randomRiddle.answer;
  riddleParaElemAnswer.style.display = "none"; //Hide the answer initially
}
...
function revealAnswers() {
  const riddleParaElemQuestion = document.getElementById(
    "riddlesParaQuestionId"
  );
  const riddleParaElemAnswer = document.getElementById("riddlesParaAnswerId");

  if (!riddleParaElemQuestion || !riddleParaElemQuestion.textContent) {
    alert("There is no riddle shown");
  } else if (riddleParaElemAnswer.style.display === "block") {
    alert("The answer is already revealed");
  } else {
    riddleParaElemAnswer.style.display = "block";
  }
}
...
/**
 * Generates a child node for the given element type and appends to the parent.
 * @param {string} parentClassName
 * @param {string} childId
 * @param {string} elementType
 * @returns
 */
function createChildElements(parentClassName, childId, elementType) {
  const parentClassNameString = "." + parentClassName;
  const parentElem = document.querySelector(parentClassNameString);
  let childIdElem = document.getElementById(childId);

  if (!childIdElem) {
    childIdElem = document.createElement(elementType);
    childIdElem.setAttribute("id", childId);
